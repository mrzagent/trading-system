#!/usr/bin/env python3
"""Final naming consistency audit - fixed version"""
import os
import re
import ast
...
def extract_registry_modules():
    """Extract STRATEGY_MODULES from strategy_registry.py using AST"""
    ...
    for node in ast.walk(tree):
        if isinstance(node, ast.Assign):
            for target in node.targets:
                if isinstance(target, ast.Name) and target.id == 'STRATEGY_MODULES':
                    # Extract list elements
                    if isinstance(node.value, ast.List):
                        modules = []
                        for elt in node.value.elts:
                            if isinstance(elt, ast.Constant):
                                modules.append(elt.value)
                        return modules
    return []

print("="*80)
print("FINAL NAMING CONSISTENCY AUDIT (FIXED)")
print("="*80)
...
if extra_in_files:
    print(f"  INFO: Extra files not in orchestrator: {sorted(extra_in_files)}")
...
discovered = get_all_strategies()
discovered_names = set()
for s in discovered:
    # Get the strategy key from metadata
    name = s.get('name', '').lower().replace(' ', '_')
    discovered_names.add(name)

# Map discovered names to orchestrator keys
name_mapping = {
    'momentum_scalper': 'momentum_scalper',
    'pullback_scalper': 'pullback_scalper',
    'rsi_confluence': 'rsi_mean_reversion',
    'momentum': 'momentum_rsi',
    'fair_value_gap_(fvg)_breakout': 'fvg_proximity',
...
mapped_discovered = set(name_mapping.get(n, n) for n in discovered_names)
missing_in_registry = orchestrator_set - mapped_discovered
if missing_in_registry:
    print(f"  FAIL: Orchestrator strategies not discovered by registry: {missing_in_registry}")
else:
    print("  PASS: All orchestrator strategies discovered by registry")
...
print(f"Registry discovered:         {len(discovered)}")
