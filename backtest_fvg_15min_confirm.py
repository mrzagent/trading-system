#!/usr/bin/env python3
"""
backtest_fvg_15min_confirm.py — FVG Strategy with Confirmation on 15-minute data

Entry: Break + 1 candle close beyond FVG (confirmation)
Wider stops: 5% to allow for volatility
"""
import sys
...
MAX_HOLD_CANDLES = 12  # 3 hours on 15m
...
PARAM_SETS = [
    ...
    {"name": "3.0 SL / 2.0 TP + Confirm", "sl": 3.0, "tp": 2.0, "confirm": True},
    {"name": "0.5 SL / 1.5 TP No Confirm", "sl": 0.5, "tp": 1.5, "confirm": False},
]
...
def run_single_backtest(candles: List[dict], params: dict) -> BacktestResult:
    ...
    position = None
    pending_entry = None  # For confirmation mode
    ...
    for i in range(warmup, len(candles)):
        ...
        # Check entry (or confirmation)
            ...
            if params["confirm"] and pending_entry:
                # We have a pending entry from previous candle - check if confirmed
                direction = pending_entry["direction"]
                fvg = pending_entry["fvg"]
                
                # Check if current candle confirms the break
                if direction == "SHORT":
                    # Bullish FVG break - need close below FVG bottom
                    ...
                    # Bearish FVG break - need close above FVG top
                ...
                pending_entry = None  # Clear pending regardless
            
            elif not params["confirm"]:
                # No confirmation needed - enter immediately on break
...
def run_backtest():
    print("=" * 70)
    print("FVG 15-MINUTE WITH CONFIRMATION + WIDER STOPS")
    print("=" * 70)
    print()
    ...
    print("=" * 70)
    print(f"{'Config':<30} {'P&L':>12} {'Win%':>8} {'PF':>6} {'Trades':>8}")
    print("-" * 70)
    for r in sorted(results, key=lambda x: x.total_pnl_usd, reverse=True):
        print(f"{r.name:<30} {r.total_pnl_usd:>+11,.0f} {r.win_rate:>7.1f}% {r.profit_factor:>6.2f} {r.total_trades:>8}")
