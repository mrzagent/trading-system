#!/usr/bin/env python3
"""Place $50 ETH trade on agent wallet - CORRECTED"""
import os
import sys
...
print("=" * 60)
print("PLACING $50 ETH TRADE ON AGENT WALLET")
print("=" * 60)
...
# Get ETH price and calculate size (4 decimals for ETH)
mids = info.all_mids()
eth_price = float(mids.get('ETH', 0))
sz = round(50 / eth_price, 4)  # 4 decimals for ETH

print(f"\nETH Price: ${eth_price:,.2f}")
print(f"Size: {sz} ETH")
print(f"Notional: ~${sz * eth_price:.2f}")
...
if pos_count_after > pos_count_before:
    ...
    print("\n[CHECK] Position count unchanged")
