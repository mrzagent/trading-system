#!/usr/bin/env python3
"""Place SOL, BTC, ETH trades on agent wallet - FIXED VERSION"""
import os
import sys

# FIX: Override process environment variables BEFORE importing anything else
...
# Create account from env (now overridden)
account = Account.from_key(os.environ['HYPERLIQUID_PRIVATE_KEY'])
WALLET = os.environ['HYPERLIQUID_WALLET']

print("=" * 60)
print("OPENING TRADES ON AGENT WALLET (FIXED)")
print("=" * 60)
print(f"Wallet: {WALLET}")

# Verify wallet matches key
derived_address = account.address
print(f"Derived from key: {derived_address}")
print(f"Match: {derived_address.lower() == WALLET.lower()}")
print()
