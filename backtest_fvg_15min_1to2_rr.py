#!/usr/bin/env python3
"""
backtest_fvg_15min_1to2_rr.py — FVG Strategy with 1:2 Risk/Reward
Hold until SL or TP hit (no time exit)
"""
import sys
...
# 1:2 Risk/Reward parameter sets
PARAM_SETS = [
    {"name": "1.0 SL / 2.0 TP (1:2)", "sl": 1.0, "tp": 2.0, "confirm": True},
    {"name": "1.5 SL / 3.0 TP (1:2)", "sl": 1.5, "tp": 3.0, "confirm": True},
    {"name": "2.0 SL / 4.0 TP (1:2)", "sl": 2.0, "tp": 4.0, "confirm": True},
    {"name": "2.5 SL / 5.0 TP (1:2)", "sl": 2.5, "tp": 5.0, "confirm": True},
    {"name": "3.0 SL / 6.0 TP (1:2)", "sl": 3.0, "tp": 6.0, "confirm": True},
    {"name": "4.0 SL / 8.0 TP (1:2)", "sl": 4.0, "tp": 8.0, "confirm": True},
]
...
def run_single_backtest(candles: List[dict], params: dict) -> BacktestResult:
    ...
    for i in range(warmup, len(candles)):
        ...
        # Check entry (or confirmation)
        else:
            if params["confirm"] and pending_entry:
                direction = pending_entry["direction"]
                fvg = pending_entry["fvg"]
            ...
            # Check for new breakout to set pending (for confirmation mode)
            if params["confirm"] and not position and not pending_entry:
                fvgs = detect_fvgs(candles, i)
                action, fvg = check_fvg_breakout(current_price, fvgs)
...
def run_backtest():
    print("=" * 70)
    print("FVG 15-MINUTE: 1:2 RISK/REWARD (NO TIME EXIT)")
    print("=" * 70)
    print()
    ...
    print("Running 1:2 R:R parameter sweep...")
    results = []
    ...
    # Summary table
    print("\n" + "=" * 70)
    print("SUMMARY COMPARISON (1:2 Risk/Reward)")
    print("=" * 70)
    print(f"{'Config':<30} {'P&L':>12} {'Win%':>8} {'PF':>6} {'Trades':>8} {'AvgBars':>8}")
    print("-" * 70)
    for r in sorted(results, key=lambda x: x.total_pnl_usd, reverse=True):
        print(f"{r.name:<30} {r.total_pnl_usd:>+11,.0f} {r.win_rate:>7.1f}% {r.profit_factor:>6.2f} {r.total_trades:>8} {r.avg_bars_held:>8.1f}")
    ...
    print(f"{'Config':<25} {'Risk':>8} {'Reward':>8} {'Win%':>8} {'Exp.Value':>10}")
    print("-" * 65)
    for r in results:
        notional = 20 * 3  # $60
        ...
        print(f"{r.name:<25} ${risk:>7.2f} ${reward:>7.2f} {r.win_rate:>7.1f}% ${exp_value:>+9.2f}")
