#!/usr/bin/env python3
"""Test the new Oversold FVG Bounce strategy - with timeframe-specific parameters"""
...
def run_test(timeframe, table, rsi_max, min_fvg=1):
    print(f"\n{'='*60}")
    print(f"Testing: {timeframe} (table: {table})")
    print(f"Strategy: RSI < {rsi_max}, FVG >= {min_fvg}")
...
if __name__ == "__main__":
    ...
    # Test with different RSI thresholds per timeframe
    # 5min: RSI < 35 (short-term oversold)
    # 1h: RSI < 40 (medium oversold)
    # 4h: RSI < 45 (relaxed for higher TF)
    
    tests = [
        ("5min", "trading_prices", 35),
        ("1h", "trading_prices_1h", 50),  # Relaxed for sparse data
        ("4h", "trading_prices_4h", 55),  # Even more relaxed
    ]
    
    for tf, table, rsi in tests:
        results[tf] = run_test(tf, table, rsi)
    
    # Summary
    print("\n" + "="*70)
    print("STRATEGY COMPARISON: Oversold FVG (timeframe-specific RSI)")
    print("="*70)
